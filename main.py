...
def main():
    ...
    # 获取已有的IP组
    ip_groups = get_ip_groups(session, iKuaiHostIP)

    # 删除包含指定组名的现有IP组
    delete_ip_groups(session, iKuaiHostIP, ip_groups, groupName)
...
def del_ipgroup_function(session, iKuaiHostIP, groupID):
    # 调用 'ipgroup' 函数删除IP组
    session.post("http://{}/Action/call".format(iKuaiHostIP), json={"func_name": "ipgroup", "action": "del",
                                                                      "param": {"id": groupID}})

def upload_ip_group(session, iKuaiHostIP, ipgroup, ip_type):
    # 上传IP组信息
    files = {'ipgroup.txt': ipgroup.encode()}
    session.post("http://{}/Action/upload".format(iKuaiHostIP), files=files)

def call_ipgroup_function(session, iKuaiHostIP, ip_type=None):
    # 调用 'ipgroup' 函数导入IP组
    json_data = {"func_name": "ipgroup", "action": "IMPORT", "param": {"filename": "ipgroup.txt", "append": 1}}
    if ip_type:
        json_data["param"]["ip_type"] = ip_type
...
def upload_and_confirm(session, iKuaiHostIP, start_group_id, start_group_name, ip_list, ip_type):
    ...
    for group_num in range(num_groups):
        ...
        del_ipgroup_function(session, iKuaiHostIP, current_group_id)

        # 上传IP组信息
        upload_ip_group(session, iKuaiHostIP, ipgroup, ip_type)

        # 根据IP类型调用 'ipgroup' 函数导入IP组
        call_ipgroup_function(session, iKuaiHostIP, ip_type)

def get_ip_groups(session, iKuaiHostIP):
    # 获取IP组列表
    response = session.post(f"http://{iKuaiHostIP}/Action/call", json={
        "func_name": "ipgroup",
        "action": "show",
...
def delete_ip_groups(session, iKuaiHostIP, ip_groups, group_name):
    # 删除指定组名的IP组
    for ip_group in ip_groups:
        gname = ip_group.get("group_name", "")
        if group_name in gname:
            group_id = ip_group.get("id", "")
            del_ipgroup_function(session, iKuaiHostIP, group_id)
